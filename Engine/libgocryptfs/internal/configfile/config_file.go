...
func Create(args *CreateArgs, returnedScryptHashBuff []byte) error {
	...
	}
	{
		...
		scryptHash, err := cf.EncryptKey(key, args.Password, args.LogN, len(returnedScryptHashBuff) > 0)
		if err != nil {
			for i := range key {
				key[i] = 0
			}
...
func (cf *ConfFile) DecryptMasterKey(password []byte, giveHash bool) (masterkey, scryptHash []byte, err error) {
	...
	scryptHash, err = cf.ScryptObject.DeriveKey(password)
	if err != nil {
		return nil, nil, err
	}
...
// cf.ScryptObject.
func (cf *ConfFile) EncryptKey(key []byte, password []byte, logN int, giveHash bool) ([]byte, error) {
